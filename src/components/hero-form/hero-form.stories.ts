...
Filled.play = async ({ canvasElement }) => {
  ...
  await userEvent.selectOptions(heroPower, 'Climate Changer');
};
...
Submitted.play = async (context) => {
  ...
  await waitFor(async () => {
    ...
    await expect(canvas.getByTestId('hero-alterego').textContent).toEqual(
      'Ororo Munroe'
    );
    await expect(canvas.getByTestId('hero-power').textContent).toEqual(
      'Climate Changer'
    );
  });
};
...
SubmittedAndEditedAfter.play = async (context) => {
  ...
  await waitFor(async () => {
    ...
    // new value
    await expect(canvas.getByTestId('hero-name').textContent).toEqual(
      'Wakanda Queen'
    );
